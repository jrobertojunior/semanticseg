...
class DataGenerator(Dataset):
    ...
    def __init__(self, data, img_size, mask_size):
        ...
        self.transform_mask = transforms.Compose(
            [transforms.Resize((self.mask_size)), toOneHot]
        )
...
class Context(object):
    ...
    def fit(
        ...
        # img_names = {x[:-4]: x[-4:] for x in img_dir}
        # mask_names = {x[:-4]: x[-4:] for x in mask_dir}

        # print(img_names)

        # extract similar names in both directories
        # names = sorted(list(set(img_names.keys()).intersection(mask_names.keys())))
        # names = img_names.keys()

        # if shuffle_data:
        #     np.random.shuffle(names)

        # split train/validation
        split_indice = int(len(img_names) * (1 - validation_split))

        root_dir = "data/headsegmentation_dataset_ccncsa"
        # for i, name in enumerate(names):
        for i in range(len(img_names)):
            if i < split_indice:
                self.train_data["img"].append(
                    root_dir + "/" + img_names[i].replace("\\", "/")
                )
                self.train_data["mask"].append(
                    root_dir + "/" + mask_names[i].replace("\\", "/")
                )
            else:
                self.valid_data["img"].append(
                    root_dir + "/" + img_names[i].replace("\\", "/")
                )
                self.valid_data["mask"].append(
                    root_dir + "/" + mask_names[i].replace("\\", "/")
                )
    ...
    def set(self, nb_epochs=5, batch_size=1):
        ...
        self.trainloader = DataLoader(
            DataGenerator(self.train_data, self.img_size, self.mask_size),
        ...
        self.validloader = DataLoader(
            DataGenerator(self.valid_data, self.img_size, self.mask_size),
            batch_size=batch_size,
            shuffle=False,
            num_workers=2,
            pin_memory=True,
    ...
    def train(self, model):
        ...
        for epoch in range(self.epochs):
            ...
            # print(enumerate(self.trainloader))
            ...
            for batch, (features, targets) in enumerate(self.trainloader):
                ...
                loss = model.criterion(
                    model(features.to(self.DEVICE)), targets.to(self.DEVICE)
                )
    ...
    def evaluate(self, model, validloader, save_loss=False):
        ...
        for i, (features, targets) in enumerate(validloader):
            ...
            ## IOU accuracy
            iou_score.append(
                iou_accuracy(model(features.to(self.DEVICE)), targets.to(self.DEVICE))
            )
        # save accuracy
        ...
        if plot:
            ...
            for i in range(axes.shape[0]):
                for j in range(1, axes.shape[1]):
                    axes[i, j].imshow(
                        (yhat).cpu().float().detach().numpy()[:, :, channel],
                        cmap="viridis",
                    )
                    channel += 1
